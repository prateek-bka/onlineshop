...
const Payment = () => {
  ...
  const total = useSelector((store) => store.CartTotals.total);
  const discount = useSelector((store) => store.CartTotals.discount);
  const coupen = useSelector((store) => store.CartTotals.coupen);
  const social = useSelector((store) => store.CartTotals.social);
  const fee = useSelector((store) => store.CartTotals.fee);
  const total_Amount = useSelector((store) => store.CartTotals.total_Amount);

  console.log(fee, total_Amount);
  ...
  const formik = useFormik({
    ...
    validationSchema: yup.object({
      card: yup
        ...
        .min(12)
        .max(12, "Please enter valid card number"),
      name: yup
        .string()
        .required("card name Required")
        .min(4)
        .max(20, "Please enter card name"),
      valid: yup
        .string()
        .required("card expiry date")
        .min(5)
        .max(5, "Please enter details"),
      cvv: yup
        .string()
        .required("cvv Required")
        .min(3)
        .max(3, "Please enter valid cvv"),
    }),
      ...
      <Flex flexDirection={["column", "column", "column", "row"]}>
        <Box margin="0 auto">
          ...
          >
            ...
            <Flex width="100%">
              <Text>
                10% Instant Discount on YES Bank Credit Cards on a min spend of
                Rs 3,000. TCA
              </Text>
            </Flex>
          </VStack>
          ...
          >
            ...
            <Flex width="100%" border="1px solid black">
              ...
              >
                ...
                <Text>
                  Please ensure Your card be used for online transactions
                </Text>
                ...
                >
                  <FormControl
                    isInvalid={formik.errors.card && formik.touched.card}
                  >
                  ...
                  <FormControl
                    isInvalid={formik.errors.valid && formik.touched.valid}
                  >
                    <Input
                      mt={["5px", "10px", "10px", "10px"]}
                      border="1px solid black"
