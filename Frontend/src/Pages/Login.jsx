...
const Login = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: (values, actions) => {
      ...
      fetch("https://tame-lime-marlin-wear.cyclic.app/user/login", {
        method: "POST",
        ...
        .then((res) => {
          res.msg === "Login successfull"
            ? localStorage.setItem("num", res.num)
            : navigate("/signup");
          Setseen(false);
        })
  ...
  const finalLogin = () => {
    ...
    fetch("https://tame-lime-marlin-wear.cyclic.app/user/login1", {
      method: "POST",
      body: JSON.stringify(payload),
      headers: {
        "Content-type": "application/json",
      ...
      .then((res) => {
        ...
        if (res.msg) {
          ...
          console.log(res);
          dispatch(LoginFunctionSuccess({ name: res.name }));
        }
        console.log(res);
        if (res.err) {
          swal("please enter correct password");
        } else {
          swal("login Successfull", "you are being redirected", "success");
          navigate(-1);
        }
      })
      .catch((err) => console.log(err));
    ...
    <Box bg="#fdefec" height="100vh">
      <Box mt="100px" display={seen === true ? "inline-block" : "none"}>
        ...
        >
          ...
          <Heading size={["xs", "xs", "sm", "lg"]} mt="10px" color={"#342c34"}>
            Login{" "}
            <Text display="inline-block" size={["sm", "sm", "md", "md"]}>
              {" "}
              or{" "}
            </Text>{" "}
            Signup
          ...
          <Button
            ...
            width="100%"
            bg="pink.500"
            _hover={{ bgColor: "pink.500", color: "white" }}
          >
            Continue
          </Button>
      ...
      <Box mt="100px" display={seen === false ? "inline-block" : "none"}>
        ...
        >
          <Image src="https://assets.myntassets.com/dpr_1.5,q_60,w_400,c_limit,fl_progressive/assets/images/2023/2/7/59a76460-3a85-4d4b-b517-faef119c50551675792734635-offer-banner-200-600x240-code-_-MYNTRA200.jpg" />
          <Heading size={["sm", "md", "md", "md"]} mt="10px">
            Enter your password
          </Heading>
